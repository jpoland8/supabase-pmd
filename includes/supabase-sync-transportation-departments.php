...
function sync_transportation_department_to_supabase($post_id, $post, $update) {
    ...
    $data = array(
        ...
        'use_transportation_department_name' => get_post_meta($post_id, 'use_transportation_department_name', true) ? 'Yes' : 'No',
        'contact_title' => get_post_meta($post_id, 'contact_title', true),
        'contact_name' => get_post_meta($post_id, 'contact_name', true),
        'contact_email' => get_post_meta($post_id, 'contact_email', true),
        'cc_emails' => get_post_meta($post_id, 'cc_emails', true),
        'phone' => get_post_meta($post_id, 'phone', true),
    );

    // Get the repeater field data for ads
    if (have_rows('ads', $post_id)) {
        while (have_rows('ads', $post_id)) {
            the_row();
            $ad = get_sub_field('ad');
    ...
    $url = rtrim(SUPABASE_FUNCTION_URL, '/') . '/rest/v1/transportation_dept?post_id=eq.' . $post_id;
    ...
    if (!empty($existing_data)) {
        ...
        // Create new record
        $url = rtrim(SUPABASE_FUNCTION_URL, '/') . '/rest/v1/transportation_dept';
        $method = 'POST';
    }
...
function delete_transportation_department_from_supabase($post_id) {
    // Check if the post is a transportation department
    $post_type = get_post_type($post_id);
    if ($post_type !== 'trans_dept') {
        return;
    }

    // Always using the "transportation_dept" table
    $url = rtrim(SUPABASE_FUNCTION_URL, '/') . '/rest/v1/transportation_dept?post_id=eq.' . $post_id;
